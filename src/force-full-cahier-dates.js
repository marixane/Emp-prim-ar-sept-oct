...
const addYear = (text) => {
  const normalized = String(text || '').replace(DUPLICATED_YEAR_FIRST_DATE_PATTERN, '$1/$2/$3');
  if (YEAR_FIRST_DATE_PATTERN.test(normalized)) return normalized;
  return normalized.replace(DATE_PATTERN, (_, day, month) => {
...
const restoreMawlidAsNormalSaturday = () => {
  ...
  // Une vraie fiche de séances du samedi a déjà été produite par Tab.jsx.
  // La conserver telle quelle au lieu de copier artificiellement le samedi suivant.
  if (target.querySelector('.cahier-session-row')) {
...
const removeNormalExitEntry = () => {
  ...
  entries.forEach((entry) => {
    ...
    if (isSameDate && !isExitEvent) entry.classList.add('cahier-dom-hidden');
  });
};
