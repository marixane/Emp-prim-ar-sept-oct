...
const exportPdf = async (button, mode = 'download') => {
  ...
  if (mode === 'preview' && button.dataset.readyPdfUrl) {
    ...
    button.textContent = button.dataset.previewIdleLabel || 'معاينة PDF';
...
const styleButton = (button, side) => {
  ...
  const bottomPosition = side === 'left' ? 236 : 82;
